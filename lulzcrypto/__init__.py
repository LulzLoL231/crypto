...
class Crypto:
    ...
    def _UploadToPastebin(self, text: str) -> str:
        ...
        Returns:
            str: string with URL to pastebin paste.

        Raises:
            `PastebinError` for any API error.'''
        data = {'api_dev_key': self.pastebin_key,
                'api_option': 'paste',
                'api_paste_code': text,
                'api_paste_private': 2}
    ...
    def encrypt(self, text: str, upload: bool = True) -> str:
        ...
        if (upload) and (self.pastebin_key):
    ...
    def decrypt(self, text: str) -> str:
        ...
        Returns:
            string with decrypted text.'''
        if text is None:
            raise VarError('"text" var is not provided.')
        else:
            if text.isdigit():
                enc_text = text
            else:
                pastebin = text
            if enc_text:
                fr = self._DecodeFromLulzCode(enc_text)
